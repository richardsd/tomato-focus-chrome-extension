...
| U1 | Sidebar navigation + detail panels expose all primary sections | **Pass** if users can reach timer/tasks/statistics/settings with clear active-state feedback. **Fail** if any section is hidden/ambiguous. | **Pass** if sidebar-driven navigation works with equivalent feature reachability and section clarity. **Fail** if a section or state is not discoverable. | Screen capture of each section + active state highlight | No | Pending |
| U2 | Timer-first main screen supports one-click primary control | **Pass** if start/pause/resume/reset/quick-start actions are accessible in one click and visibly tied to current session state. **Fail** if actions require extra navigation or unclear state mapping. | **Pass** if modern timer screen preserves the same action semantics while improving legibility and control prominence. **Fail** on semantic drift or poor discoverability. | Action timing video + state labels at each transition | Yes | Pending |
| U3 | Visual theme application respects system/light/dark preference | **Pass** if extension theme setting maps correctly to system/light/dark behavior in all major surfaces. **Fail** if preference is ignored/inconsistently applied. | **Pass** if macOS shell and feature views honor `system`, `light`, and `dark` settings consistently. **Fail** when surfaces or controls diverge from selected theme. | Theme snapshots for each mode across all sections | No | Pending |
